...
template<typename T, typename S>
__host__ libstl::vector<T>* p_spmv_csr_balanced_host(const CSR_matrix<T>& mtx, const libstl::vector<T>& v, T& zero, size_t gridSize, size_t blockSize)
{
    ...
    (
        [=]
        __device__ (const CSR_matrix<T>& mtx, const libstl::vector<T>& v)
        {
    ...
    for(size_t i=0; i<mtx_row_size; i++)
    {
        ...
        (
            ...
            __device__ (const CSR_matrix<T>& mtx, const libstl::vector<T>& v)
            {
                ...
                while(idx < e)
                {
                    (*p_max_mid_res)[tid] = (*p_max_mid_res)[tid] + v[mtx.col_idx[idx]] * mtx.data[idx];
                    idx += tnum;
                }
            }, mtx, v
        );
    ...
    for(size_t i=0; i < mtx_row_size; i++)
    {
        ...
        (
            ...
            __device__ (const CSR_matrix_opt<T>& mtx, const libstl::vector<T>& vec, T& zero, unsigned char* smem)
            {
                ...
                while(idx < e)
                {    
                    s_mid_res[bid] = s_mid_res[bid] + v[mtx.col_idx[idx]] * mtx.data[idx];
                    idx += tnum;
                }
                __syncthreads();
...
template<typename T>
__host__ libstl::vector<T>* p_spmv_csr_balanced_host(const CSR_matrix<T>& mtx, const libstl::vector<T>& vec, T& zero, size_t gridSize, size_t blockSize)
{
    return p_spmv_csr_balanced_host<T, T>(mtx, vec, zero, gridSize, blockSize);
}
