#ifndef __MNT4_PAIRING_CU__
#define __MNT4_PAIRING_CU__
...
__device__ bool mnt4_ate_G1_precomp::operator==(const mnt4_ate_G1_precomp &other) const
{
...
__device__ bool mnt4_ate_dbl_coeffs::operator==(const mnt4_ate_dbl_coeffs &other) const
{
...
__device__ bool mnt4_ate_add_coeffs::operator==(const mnt4_ate_add_coeffs &other) const
{
    return (this->c_L1 == other.c_L1 &&
            this->c_RZ == other.c_RZ);
}

__device__ bool mnt4_ate_G2_precomp::operator==(const mnt4_ate_G2_precomp &other) const
{
...
__device__ mnt4_Fq4 mnt4_final_exponentiation_last_chunk(const mnt4_Fq4 &elt, const mnt4_Fq4 &elt_inv)
{
...
__device__ mnt4_Fq4 mnt4_final_exponentiation_first_chunk(const mnt4_Fq4 &elt, const mnt4_Fq4 &elt_inv)
{
    /* (q^2-1) */
...
__device__ mnt4_affine_ate_G2_precomputation mnt4_affine_ate_precompute_G2(const mnt4_G2& Q)
{
    ...
    long* NAF;
    size_t NAF_size;
    find_wnaf(&NAF, &NAF_size, 1, *mnt4_ate_loop_count);
    ...
    result.coeffs = new mnt4_affine_ate_coeffs[result.coeffs_size];
                                                ...
                                                const mnt4_affine_ate_G2_precomputation &prec_Q)
{
    ...
    for (long i = NAF_size - 1; i >= 0; --i)
    {
        ...
        mnt4_Fq4 g_RR_at_P = mnt4_Fq4(&mnt4_fp4_params_q, prec_P.PY_twist_squared, - prec_P.PX * c.gamma_twist + c.gamma_X - c.old_RY);
        ...
        if (NAF[i] != 0)
        {
            ...
            if (NAF[i] > 0)
                g_RQ_at_P = mnt4_Fq4(&mnt4_fp4_params_q, prec_P.PY_twist_squared, - prec_P.PX * c.gamma_twist + c.gamma_X - prec_Q.QY);
            else
                g_RQ_at_P = mnt4_Fq4(&mnt4_fp4_params_q, prec_P.PY_twist_squared, - prec_P.PX * c.gamma_twist + c.gamma_X + prec_Q.QY);
...
__device__ void doubling_step_for_flipped_miller_loop(extended_mnt4_G2_projective &current, mnt4_ate_dbl_coeffs &dc)
{
    ...
    const mnt4_Fq2 F = (B.dbl() + B) + *mnt4_twist_coeff_a * A;  // F = 3*B +  a  *A
    const mnt4_Fq2 G = F.squared(); // G = F^2

    current.X = -(E.dbl().dbl()) + G; // X3 = -4*E+G
    current.Y = -mnt4_Fq(&mnt4_fp_params_q, "8") * D + F * (E.dbl() - current.X); // Y3 = -8*D+F*(2*E-X3)
...
__device__ void mixed_addition_step_for_flipped_miller_loop(const mnt4_Fq2 base_X, const mnt4_Fq2 base_Y, const mnt4_Fq2 base_Y_squared,
                                                            ...
                                                            mnt4_ate_add_coeffs &ac)
{
    ...
    const mnt4_Fq2 E = I.dbl().dbl(); // E = 4*I
    const mnt4_Fq2 J = H * E;         // J = H * E
    const mnt4_Fq2 V = X1 * E;        // V = X1 * E
    const mnt4_Fq2 L1 = D - Y1.dbl(); // L1 = D - 2 * Y1
...
__device__ mnt4_ate_G2_precomp mnt4_ate_precompute_G2(const mnt4_G2& Q)
{
    ...
    const bigint<mnt4_r_limbs> &loop_count = *mnt4_ate_loop_count;
    ...
    result.dbl_coeffs = new mnt4_ate_dbl_coeffs[result.dbl_coeffs_size];
    result.add_coeffs = new mnt4_ate_add_coeffs[result.add_coeffs_size];
...
__device__ mnt4_Fq4 mnt4_ate_miller_loop(const mnt4_ate_G1_precomp &prec_P, const mnt4_ate_G2_precomp &prec_Q)
{
    mnt4_Fq  t(&mnt4_fp_params_q);
    mnt4_Fq4 t4(&mnt4_fp4_params_q);

    mnt4_Fq2 L1_coeff = mnt4_Fq2(&mnt4_fp2_params_q, prec_P.PX, t.zero()) - prec_Q.QX_over_twist;
    ...
    for (long i = loop_count.max_bits() - 1; i >= 0; --i)
    {
        ...
        mnt4_Fq4 g_RR_at_P = mnt4_Fq4(&mnt4_fp4_params_q, - dc.c_4C - dc.c_J * prec_P.PX_twist + dc.c_L,  dc.c_H * prec_P.PY_twist);
        f = f.squared() * g_RR_at_P;

        if (bit)
        {
            mnt4_ate_add_coeffs ac = prec_Q.add_coeffs[add_idx++];
            mnt4_Fq4 g_RQ_at_P = mnt4_Fq4(&mnt4_fp4_params_q, ac.c_RZ * prec_P.PY_twist, -(prec_Q.QY_over_twist * ac.c_RZ + L1_coeff * ac.c_L1));
            f = f * g_RQ_at_P;
        }
    }
    ...
    {
    	mnt4_ate_add_coeffs ac = prec_Q.add_coeffs[add_idx++];
    	mnt4_Fq4 g_RnegR_at_P = mnt4_Fq4(&mnt4_fp4_params_q, ac.c_RZ * prec_P.PY_twist, -(prec_Q.QY_over_twist * ac.c_RZ + L1_coeff * ac.c_L1));
                                                ...
                                                const mnt4_ate_G2_precomp &prec_Q2)
{
    ...
    mnt4_Fq2 L1_coeff1 = mnt4_Fq2(&mnt4_fp2_params_q, prec_P1.PX, t.zero()) - prec_Q1.QX_over_twist;
    mnt4_Fq2 L1_coeff2 = mnt4_Fq2(&mnt4_fp2_params_q, prec_P2.PX, t.zero()) - prec_Q2.QX_over_twist;
    ...
    for (long i = loop_count.max_bits() - 1; i >= 0; --i)
    {
        ...
        mnt4_Fq4 g_RR_at_P1 = mnt4_Fq4(&mnt4_fp4_params_q, - dc1.c_4C - dc1.c_J * prec_P1.PX_twist + dc1.c_L, dc1.c_H * prec_P1.PY_twist);
        mnt4_Fq4 g_RR_at_P2 = mnt4_Fq4(&mnt4_fp4_params_q, - dc2.c_4C - dc2.c_J * prec_P2.PX_twist + dc2.c_L, dc2.c_H * prec_P2.PY_twist);
        ...
        if (bit)
        {
            ...
            mnt4_Fq4 g_RQ_at_P1 = mnt4_Fq4(&mnt4_fp4_params_q, ac1.c_RZ * prec_P1.PY_twist, -(prec_Q1.QY_over_twist * ac1.c_RZ + L1_coeff1 * ac1.c_L1));
            mnt4_Fq4 g_RQ_at_P2 = mnt4_Fq4(&mnt4_fp4_params_q, ac2.c_RZ * prec_P2.PY_twist, -(prec_Q2.QY_over_twist * ac2.c_RZ + L1_coeff2 * ac2.c_L1));
    ...
    if (mnt4_ate_is_loop_count_neg)
    {
        ...
        mnt4_ate_add_coeffs ac2 = prec_Q2.add_coeffs[add_idx];
    	++add_idx;
    	mnt4_Fq4 g_RnegR_at_P1 = mnt4_Fq4(&mnt4_fp4_params_q, ac1.c_RZ * prec_P1.PY_twist, -(prec_Q1.QY_over_twist * ac1.c_RZ + L1_coeff1 * ac1.c_L1));
    	mnt4_Fq4 g_RnegR_at_P2 = mnt4_Fq4(&mnt4_fp4_params_q, ac2.c_RZ * prec_P2.PY_twist, -(prec_Q2.QY_over_twist * ac2.c_RZ + L1_coeff2 * ac2.c_L1));
...
__device__ mnt4_Fq4 mnt4_miller_loop(const mnt4_G1_precomp &prec_P, const mnt4_G2_precomp &prec_Q)
{
