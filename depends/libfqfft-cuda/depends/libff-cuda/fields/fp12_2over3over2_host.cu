#ifndef __FP12_2OVER3OVER2_HOST_CU__
#define __FP12_2OVER3OVER2_HOST_CU__

namespace libff {
    
template<mp_size_t n> 
Fp6_3over2_model_host<n> Fp12_2over3over2_model_host<n>::mul_by_non_residue(const Fp6_3over2_model_host<n>& elt) const
{
    my_Fp2 non_residue(params->fp6_params->fp2_params, *params->non_residue_c0, *params->non_residue_c1);
    return Fp6_3over2_model_host<n>(params->fp6_params, non_residue * elt.c2, elt.c0, elt.c1);
}

template<mp_size_t n> 
bool Fp12_2over3over2_model_host<n>::operator==(const Fp12_2over3over2_model_host<n>& other) const
{
...
template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::operator+(const Fp12_2over3over2_model_host<n>& other) const
{
    return Fp12_2over3over2_model_host<n>(params, this->c0 + other.c0, this->c1 + other.c1);
}
...
{
    return Fp12_2over3over2_model_host<n>(rhs.params, lhs * rhs.c0, lhs * rhs.c1);
}
...
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::operator*(const Fp12_2over3over2_model_host<n>& other) const
{
    ...
    return Fp12_2over3over2_model_host<n>(params, aA + this->mul_by_non_residue(bB), (a + b) * (A + B) - aA - bB);
}

template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::operator-() const
{
    return Fp12_2over3over2_model_host<n>(params, -this->c0, -this->c1);
}

template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::mul_by_024(const Fp2_model_host<n>& ell_0, const Fp2_model_host<n>& ell_VW, const Fp2_model_host<n>& ell_VV) const
{
    ...
    */
    ...
    return Fp12_2over3over2_model_host<n>(params, my_Fp6(params->fp6_params, z0,z1,z2),my_Fp6(params->fp6_params, z3,z4,z5));

}

template<mp_size_t n>
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::mul_by_045(const Fp2_model_host<n>& ell_0, const Fp2_model_host<n>& ell_VW, const Fp2_model_host<n>& ell_VV) const
{
    /*
    // OLD
    ...
    */
    ...
    return Fp12_2over3over2_model_host<n>(params, my_Fp6(params->fp6_params, t0, t1, t2), my_Fp6(params->fp6_params, t3, t4, t5));
}

template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::dbl() const
{
    return Fp12_2over3over2_model_host<n>(params, this->c0.dbl(), this->c1.dbl());
}

template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::squared() const
{
    return squared_complex();
}

template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::squared_karatsuba() const
{
    ...
    return Fp12_2over3over2_model_host<n>(params, asq + this->mul_by_non_residue(bsq), (a + b).squared() - asq - bsq);
}

template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::squared_complex() const
{
    ...
    return Fp12_2over3over2_model_host<n>(params, (a + b) * (a + this->mul_by_non_residue(b)) - ab - this->mul_by_non_residue(ab), ab.dbl());
}

template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::inverse() const
{
    ...
    return Fp12_2over3over2_model_host<n>(params, c0, c1);
}

template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::Frobenius_map(unsigned long power) const
{
    ...
    return Fp12_2over3over2_model_host<n>(params, c0.Frobenius_map(power), Frobenius_coeffs_c1[power % 12] * c1.Frobenius_map(power));
}

template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::unitary_inverse() const
{
    return Fp12_2over3over2_model_host<n>(params, this->c0, -this->c1);
}

template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::cyclotomic_squared() const
{
    /* OLD: naive implementation
       return (*this).squared();
    */
    ...
    return Fp12_2over3over2_model_host<n>(params, my_Fp6(params->fp6_params, z0,z4,z3),my_Fp6(params->fp6_params, z2,z1,z5));
}



template<mp_size_t n> 
void Fp12_2over3over2_model_host<n>::set_params(Fp12_params_host<n>* params)
{
    this->params = params;
...
template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::zero()
{
    my_Fp6 t(params->fp6_params);
    return Fp12_2over3over2_model_host<n>(params, t.zero(), t.zero());
}

template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::one()
{
    my_Fp6 t(params->fp6_params);
    return Fp12_2over3over2_model_host<n>(params, t.one(), t.zero());
}

template<mp_size_t n> 
Fp12_2over3over2_model_host<n> Fp12_2over3over2_model_host<n>::random_element()
{
    my_Fp6 t(params->params);

    Fp12_2over3over2_model_host<n> r(params);
