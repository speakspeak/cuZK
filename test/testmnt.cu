...
#include "../depends/libff-cuda/curves/mnt4/mnt4_init_host.cuh"
#include "../depends/libff-cuda/curves/mnt4/mnt4_g1_host.cuh"
#include "../depends/libff-cuda/curves/mnt4/mnt4_g2_host.cuh"
#include "../depends/libff-cuda/curves/mnt4/mnt4_pp_host.cuh"
...
#include "../depends/libff-cuda/curves/mnt4/mnt4_init.cuh"
#include "../depends/libff-cuda/curves/mnt4/mnt4_pp.cuh"
...
struct h_instance_params
{
    mnt4_Fr_host h_instance;
    mnt4_G1_host h_g1_instance;
    mnt4_G2_host h_g2_instance;
    mnt4_GT_host h_gt_instance;
};
...
{
    ip->instance = mnt4_Fr(&mnt4_fp_params_r);
    ip->g1_instance = mnt4_G1(&g1_params);
    ip->g2_instance = mnt4_G2(&g2_params);
    ip->gt_instance = mnt4_GT(&mnt4_fp4_params_q);
}

void instance_init_host(h_instance_params* ip)
{
    ip->h_instance = mnt4_Fr_host(&mnt4_fp_params_r_host);
    ip->h_g1_instance = mnt4_G1_host(&g1_params_host);
    ip->h_g2_instance = mnt4_G2_host(&g2_params_host);
    ip->h_gt_instance = mnt4_GT_host(&mnt4_fp4_params_q_host);
}
...
template<typename FieldT>
__global__ void generate_r1cs_construction(r1cs_params<mnt4_Fr>* rp, const instance_params* ip, size_t log_size)
{
...
int main(int argc, char* argv[])
{
    ...
    mnt4_pp_host::init_public_params();
    ...
    r1cs_params<mnt4_Fr>* rp[deviceCount];

    for(size_t i=0; i<deviceCount; i++)
    {
        cudaSetDevice(i);
        if( cudaMalloc( (void**)&rp[i], sizeof(r1cs_params<mnt4_Fr>)) != cudaSuccess) printf("rp malloc error!\n");
        generate_r1cs_construction<mnt4_Fr><<<1, 1>>>(rp[i], ip[i], log_size);
    }
    for(size_t i=0; i<deviceCount; i++)
    {
        cudaSetDevice(i);
    ...
    generator_params<mnt4_pp>* gp[deviceCount];
    for(size_t i=0; i<deviceCount; i++)
    {
        cudaSetDevice(i);
        if( cudaMalloc( (void**)&gp[i], sizeof(generator_params<mnt4_pp>)) != cudaSuccess) printf("gp malloc error!\n");
        generator_init<<<1, 1>>>(gp[i]);
        cudaDeviceSynchronize();
        groth_generator<mnt4_pp>(gp[i], rp[i], ip[i]);
    }
    for(size_t i=0; i<deviceCount; i++)
    {
        cudaSetDevice(i);
    ...
    groth_keypair_host<mnt4_pp_host> hkp;
    groth_keypair_device2host<mnt4_pp_host, mnt4_pp>(&hkp, &gp[0]->kp);
    r1cs_constraint_system_host<mnt4_Fr_host> hcs;
    r1cs_constraint_system_device2host<mnt4_Fr_host, mnt4_Fr>(&hcs, &rp[0]->r1cs.constraint_system);
    r1cs_primary_input_host<mnt4_Fr_host> hpi;
    r1cs_primary_input_device2host<mnt4_Fr_host, mnt4_Fr>(&hpi, &rp[0]->r1cs.r1cs_primary_input);
    r1cs_auxiliary_input_host<mnt4_Fr_host> hai;
    r1cs_auxiliary_input_device2host<mnt4_Fr_host, mnt4_Fr>(&hai, &rp[0]->r1cs.r1cs_auxiliary_input);
    ...
    groth_prover<mnt4_pp_host, mnt4_pp>(&hcs, &hkp.pk, &hpi, &hai, ip, &hip);
