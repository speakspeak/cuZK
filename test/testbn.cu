...
#include "../depends/libff-cuda/curves/alt_bn128/alt_bn128_init_host.cuh"
#include "../depends/libff-cuda/curves/alt_bn128/alt_bn128_g1_host.cuh"
#include "../depends/libff-cuda/curves/alt_bn128/alt_bn128_g2_host.cuh"
#include "../depends/libff-cuda/curves/alt_bn128/alt_bn128_pp_host.cuh"
...
struct instance_params
{
    alt_bn128_Fr instance;
    alt_bn128_G1 g1_instance;
    alt_bn128_G2 g2_instance;
    alt_bn128_GT gt_instance;
};

struct h_instance_params
{
    alt_bn128_Fr_host h_instance;
    alt_bn128_G1_host h_g1_instance;
    alt_bn128_G2_host h_g2_instance;
    alt_bn128_GT_host h_gt_instance;
};
...
{
    ip->instance = alt_bn128_Fr(&alt_bn128_fp_params_r);
    ip->g1_instance = alt_bn128_G1(&g1_params);
    ip->g2_instance = alt_bn128_G2(&g2_params);
    ip->gt_instance = alt_bn128_GT(&alt_bn128_fp12_params_q);
}

void instance_init_host(h_instance_params* ip)
{
    ip->h_instance = alt_bn128_Fr_host(&alt_bn128_fp_params_r_host);
    ip->h_g1_instance = alt_bn128_G1_host(&g1_params_host);
    ip->h_g2_instance = alt_bn128_G2_host(&g2_params_host);
    ip->h_gt_instance = alt_bn128_GT_host(&alt_bn128_fp12_params_q_host);
}
...
template<typename FieldT>
__global__ void generate_r1cs_construction(r1cs_params<alt_bn128_Fr>* rp, const instance_params* ip, size_t log_size)
{
...
int main(int argc, char* argv[])
{
    ...
    alt_bn128_pp_host::init_public_params();
    ...
    // r1cs construction
    r1cs_params<alt_bn128_Fr>* rp;
    if( cudaMalloc( (void**)&rp, sizeof(r1cs_params<alt_bn128_Fr>)) != cudaSuccess) printf("rp malloc error!\n");

    generate_r1cs_construction<alt_bn128_Fr><<<1, 1>>>(rp, ip[0], log_size);

    r1cs_params<alt_bn128_Fr>* rpt[deviceCount];

    for(size_t i=1; i<deviceCount; i++)
    {
        cudaSetDevice(i);
        if( cudaMalloc( (void**)&rpt[i], sizeof(r1cs_params<alt_bn128_Fr>)) != cudaSuccess) printf("rp malloc error!\n");
        generate_r1cs_construction<alt_bn128_Fr><<<1, 1>>>(rpt[i], ip[i], log_size);
    }

    cudaSetDevice(0);

    // generator
    generator_params<alt_bn128_pp>* gp;
    if( cudaMalloc( (void**)&gp, sizeof(generator_params<alt_bn128_pp>)) != cudaSuccess) printf("gp malloc error!\n");
    generator_init<<<1, 1>>>(gp);
    cudaDeviceSynchronize();
    groth_generator<alt_bn128_pp>(gp, rp, ip[0]);

    generator_params<alt_bn128_pp>* gpt[deviceCount];
    for(size_t i=1; i<deviceCount; i++)
    {
        cudaSetDevice(i);
        if( cudaMalloc( (void**)&gpt[i], sizeof(generator_params<alt_bn128_pp>)) != cudaSuccess) printf("gp malloc error!\n");
        generator_init<<<1, 1>>>(gpt[i]);
        cudaDeviceSynchronize();
        groth_generator<alt_bn128_pp>(gpt[i], rpt[i], ip[i]);
    }

    cudaSetDevice(0);


    groth_keypair_host<alt_bn128_pp_host> hkp;
    groth_keypair_device2host<alt_bn128_pp_host, alt_bn128_pp>(&hkp, &gp->kp);
    r1cs_constraint_system_host<alt_bn128_Fr_host> hcs;
    r1cs_constraint_system_device2host<alt_bn128_Fr_host, alt_bn128_Fr>(&hcs, &rp->r1cs.constraint_system);
    r1cs_primary_input_host<alt_bn128_Fr_host> hpi;
    r1cs_primary_input_device2host<alt_bn128_Fr_host, alt_bn128_Fr>(&hpi, &rp->r1cs.r1cs_primary_input);
    r1cs_auxiliary_input_host<alt_bn128_Fr_host> hai;
    r1cs_auxiliary_input_device2host<alt_bn128_Fr_host, alt_bn128_Fr>(&hai, &rp->r1cs.r1cs_auxiliary_input);
    ...
    groth_prover<alt_bn128_pp_host, alt_bn128_pp>(&hcs, &hkp.pk, &hpi, &hai, ip, &hip);
