...
int main(int argc, char* argv[])
{
    ...
    // elements generation
    MSM_params<alt_bn128_pp>* mp[deviceCount];
    for(size_t i=0; i<deviceCount; i++)
    {
        cudaSetDevice(i);
        if( cudaMalloc( (void**)&mp[i], sizeof(MSM_params<alt_bn128_pp>)) != cudaSuccess) printf("mp malloc error!\n");
        generate_MP<alt_bn128_pp><<<1, 1>>>(mp[i], ip[i], 1<<log_size);
    }
    for(size_t i=0; i<deviceCount; i++)
    {
        cudaSetDevice(i);
    ...
    // msm
    MSM<alt_bn128_pp> msm[deviceCount];
    for(size_t i=0; i<deviceCount; i++)
    {
        msm[i].device_id = i;
        msm[i].mp = mp[i];
        msm[i].ip = ip[i];
        thread[i] = start_thread( multi_MSM<alt_bn128_pp>, &msm[i] );
    }
    for(size_t i=0; i<deviceCount; i++)
    {
        end_thread(thread[i]);
    ...
    libff::G1<alt_bn128_pp_host> hg1[deviceCount];
    for(size_t i=0; i < deviceCount; i++)
    {
        cudaSetDevice(i);
        D2H<alt_bn128_pp_host, alt_bn128_pp>(&hg1[i], msm[i].res, &hip.h_g1_instance);
    }

    Reduce<alt_bn128_pp_host>(hg1, &hip.h_instance, (size_t)1<<log_size);
